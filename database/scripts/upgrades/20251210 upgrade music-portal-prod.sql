...
select system_cluster.assert_cluster_schema('prod', 'music_portal');

revoke all on table music_portal.cluster_schema from anon, authenticated;
revoke all on table music_portal.site_user from anon, authenticated;
revoke all on table music_portal.skill_level from anon, authenticated;
revoke all on table music_portal.song from anon, authenticated;
revoke all on table music_portal.user_role from anon, authenticated;
revoke all on table music_portal.user_song from anon, authenticated;
revoke all on table music_portal.user_song_measure from anon, authenticated;
revoke all on table music_portal.v_cluster_schema from anon, authenticated;
...
$$;
revoke all on function system_cluster.assert_cluster_schema(text, text) from public, authenticated, anon;
grant execute on function system_cluster.assert_cluster_schema(text, text) to service_role;
...
returns void
language plpgsql
set search_path = system_cluster
as $$
declare
    _inet_addr_expected inet;
    _inet_addr_actual   inet;
